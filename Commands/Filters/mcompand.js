...
module.exports = {
	...
	async execute(client, message) {
		...
		try { await client.distube.setFilter(message, 'mcompand'); return message.channel.send(new MessageEmbed().setColor(client.color.color).setDescription(`**Mcompand** filter applying to the music.`).setTimestamp()); } catch (e) { return message.channel.send(new MessageEmbed().setColor('RED').setDescription(`An error occurred while processing the command.`).setTimestamp()) };
	}
}